...
% III 模糊控制计算（以绿灯相位为例）
TRgreen = readfis('GreenIntensity');
...
while (cur_phase_green< max_green)&&(sim_time < sim_time_set)&&(control_list(max(cur_phase,1)))
    ,queue(cur_phase_4,1))+1,round(10*rate(floor(sim_time/300)+1),((queue(cur_phase)>=queue(cur_phase+4))*cur_phase+(queue(cur_phase)<queue(cur_phase+4))*(cur_phase+4))))+1,max(queue(((cur_phase==4)*(phaseA+4)+(cur_phase~=4)*(cur_phase+5)),1))+1)<0.5)
